...
class Game extends Component {
  ...
  shuffledImageList = () => imagesList.sort(() => Math.random() - 0.5)

  click = id => {
    const {time, list} = this.state
    const Number = Math.floor(Math.random() * 29)
    console.log(Number)
    this.setState({showObject: list[Number], showPic: list[Number].imageUrl})
            ...
            {!result ? (
              <div className="con">
                ...
                <li className="button-con">
                  <button onClick={this.onClickFruits}>Fruits</button>

                  <button onClick={this.onClickAnimals}>Animals</button>
                  <button onClick={this.onClickPlaces}>Places</button>
                </li>
                <div className="pics-con">
                  {showImgs.map(each => (
                    <Card clicked={this.click} details={each} key={each.id} />
                  ))}
                </div>
              </div>
            ) : (
